...
0.06                                 & 105                                                                            & 100                                                                           & -95                      & 9.025                          & 902,5                            \\ \hline
0.07                                 & 99                                                                             & 100                                                                           & -89                      & 7.921                          & 792,1                            \\ \hline
0.08                                 & 113                                                                            & 100                                                                           & -103                     & 10.609                         & 1.060,9                          \\ \hline
0.09                                 & 124                                                                            & 100                                                                           & -114                     & 12.996                         & 1.299,6                          \\ \hline
0.10                                 & 110                                                                            & 100                                                                           & -100                     & 10.000                         & 1.000                            \\ \hline
...
Si $X$ e $Y$ son dos variables aleatorias independientes con funciones de densidad de probabilidad $f{1}$ y $f{2}$, respectivamente, entonces la densidad de probabilidad de la suma $(X + Y) = Z$ estará dada por la convolución $f_{c}$, mostrada en la ecuación \ref{discreta}, donde $X=i$ y $Y=j-i$:

\begin{align}
f_{c} & = f{1}*f{2} \label{discreta} \\ \nonumber 
f_{c}{(x)} & = \sum_{z}f1(z) \times f2{(x-Z)}, \\ \nonumber
f_{c}{(z)} & = \sum_{x}f1(x) \times f2{(z-x)}. \\ \nonumber
\end{align}

Para el caso continuo, si $f$ y $g$ son funciones continuas,  ($f*g$)($t$), en un sistema unidimensional, se define como la integral del producto de ambas funciones después de desplazar una de ellas una distancia $t$ y se calcula mediante la ecuación \ref{continua}:
\begin{align} 
(f*g)(t) & = \int_{-\infty}^{\infty}f(t-x) \times g(x)dx = \int_{-\infty}^{\infty}f(t-y) \times g(y)dy  \label{continua} \\ \nonumber 
\end{align}

\noindent La ecuación \ref{continua} se puede reescribir mediante la la ecuación \ref{continuamod}, donde $\tau$ es la variable sobre la cual se realiza el desplazamiento:
\begin{align} 
(f*g)(t) & = \int_{-\infty}^{\infty}f(t-\tau) \times g(\tau)d\tau  \label{continuamod} \\ \nonumber 
\end{align}




Para el caso de la señales digitales, si $A{n}$ y $B{n}$ son funciones con respecto a un valor $n$ entero, entonces la convolución en un sistema unidimensional, se representa mediante la ecuación \ref{discretaseñal}:
\begin{align} 
A{[n_{a}]} * B{[n_{b}]} = C{[n_{c}]} & = \sum_{\tau=0}^{n_{a}-1} A[\tau] \times B{[n_{c}-\tau}], \label{discretaseñal} \\  \nonumber
\end{align}
\noindent donde $0 \leq n_{c} < n_{a} + n_{b} - 1$ y $\tau$ es la variable sobre la cual se realiza el desplazamiento.

Sin embargo, debido a que las imágenes son bidimensionales, es neceario la extensión a $2$-D de la ecuación \ref{discretaseñal} para realizar la convolución con imágenes, la cuál es mostrada en la ecuación \ref{discretabi}:
\begin{align} 
A{[i_{a}, \, j_{a}]} * B{[i_{b}, \, j_{b}]} = C{[i_{c}, \, j_{c}]} & = \sum_{\tau_{1}=0}^{i_{a}-1} \sum_{\tau_{2}=0}^{j_{a}-1} A[\tau_{1}, \, \tau_{2}] \times B{[i-\tau_{1}, \, j-\tau_{2}}]. \label{discretabi} \\  \nonumber
\end{align}
